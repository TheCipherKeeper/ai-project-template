...
| **Сервис** | по репо на сервис | `AGENTS.md`, `docs/{ARCHITECTURE,specs/}`, `Dockerfile`, `docker-compose.yml` (локальный) | один микросервис; клиент брокера; один из сервисов — `gateway` (каноническая роль; модель — `COMMUNICATION` → *gateway-сервис*); инстанциация из `skeletons/service/` |
...
- **gateway-сервис** — сервис с канонической ролью (назначается в `COMPOSITION`;
  ровно один, если есть ≥1 интерфейса). Модель —
  `docs/refs/COMMUNICATION.md` → *gateway-сервис*.
...
(см. `COMMUNICATION` → *gateway-сервис*); поэтому единого хаб-посредника, как у
...
- **Новый сервис:** скопируй `skeletons/service/` → новый репо → выбери стек
  ...
  (модель — `COMMUNICATION` → *gateway-сервис*). Своего `BACKLOG` у сервиса нет —
