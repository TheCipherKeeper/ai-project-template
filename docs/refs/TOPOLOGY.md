...
Топология реализует профиль `broker-microservices-v1`. Граница между этим
архитектурным выбором и универсальными agent-safety правилами описана в
`docs/refs/PROFILES.md`.
...
|---|---|---|---|
| **Методология** (этот репо) | один | `docs/guide/`, `docs/refs/`, `schemas/`, `tools/verify/`, `skeletons/`, корневые `README`/`AGENTS` | версионированный авторитет и исполняемый гейт |
...
- **Версия канона** — в `.methodology.yml` каждого репозитория. Гейт читает
  закреплённый tag, не плавающий `main`; обновление версии — отдельная задача.
- **Evidence** — неизменяемый CI artifact по `docs/refs/EVIDENCE.md`.
...
| Требование / артефакт | Хаб | Сервис | Gateway | Интерфейс | Автономный компонент |
|---|---|---|---|---|---|
| `MODULE.md` / `SPEC.md` | — | обязательно | обязательно | не применяется | не применяется |
| Брокер | системный compose и контракт | клиент | клиент | запрещён | запрещён |
| Browser-facing API | — | запрещён | предоставляет | потребляет | не применяется |
| `docs/ARCHITECTURE.md` | — | обязательно | обязательно | обязательно | обязательно |
| ADR | хранит в `adr/` | ссылается | ссылается | ссылается | ссылается |
| `BACKLOG.md` | хранит | использует | использует | использует | использует |
| `.methodology.yml` | обязательно | обязательно | обязательно | обязательно | обязательно |
| CI verify + evidence | обязательно | обязательно | обязательно | обязательно | обязательно |
| Этапы 00 / 10 / 20 / 50 | не применяются | применяются | применяются | не применяются | не применяются |
| Этапы 30 / 40 / 60 / 70 | применяются по задаче | применяются | применяются | применяются | применяются |

`—` означает, что сущность не принадлежит этому типу репозитория; «запрещён» —
явный архитектурный запрет. Подробная применимость инвариантов гейта —
