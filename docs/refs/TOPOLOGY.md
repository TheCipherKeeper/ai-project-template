...
| **Stub-таргет** | по репо на цель | `AGENTS.md`, `README.md`, `Dockerfile`, `docker-compose.yml` (локальный, без брокера), `.env.example`, `docs/ARCHITECTURE` | пассивная цель: контейнер с реальными сетевыми поверхностями (raw TCP/SSH/HTTP-баннер), параметризуется дескриптором из manage, наблюдается collector'ом out-of-band; **не** брокер-клиент, **не** peer-сервис, без presentation-эндпоинтов; инстанциация из `skeletons/stub/` |
...
- **Архитектура stub-таргета** (роль passive target, поверхности, доверительная
  граница, деплой) — в **stub-репо** (`docs/ARCHITECTURE.md`). `MODULE.md`/`SPEC.md`
  к stub не применяются (он параметризуется дескриптором, не юзкейсами).
...
- **Stub-таргет** — не участник брокера и не peer-сервис: не publish/consume
  топики, не имеет presentation-эндпоинтов, не зовёт чужие сервисы. Наблюдается
  collector'ом out-of-band; параметризуется дескриптором из manage. Деплой —
  контейнером.
...
`методология → сервис` нет). Канон интерфейса расщеплён по двум авторитетам:
...
единого хаб-посредника, как у сервисов, нет и канон тянется напрямую. **Stub-таргет
аналогичен интерфейсу**: репо-тип (passive target, без брокера/presentation, без
...
факт (collector сканирует/слушает поверхности stub'а); фиксируется в `COMPOSITION`
...
- **Новый stub-таргет:** скопируй `skeletons/stub/` → новый репо → заполни
  `docs/ARCHITECTURE.md` (роль passive target, поверхности, доверительная граница,
  деплой) → `README`. Брокера/presentation здесь нет.
