# Технологические стеки — справочник
...
> Этот центральный референс всегда хранит **все** допустимые стеки. При
> инстанциации репозитория его не копируют и не сокращают: в `AGENTS.md`
> создаваемого сервиса или автономного компонента оставляют только строку
> выбранного стека. Команды `lint / test / build` прогоняются перед коммитом —
> процедура в `docs/guide/40-verify.md`.
...
## TypeScript для пользовательского интерфейса
...
> гейт-agent `VER-014`; интерфейс потребляет только gateway).
