...
Раскладка **stub-репо** (standalone-программа; инстанцируется из `skeletons/stub/`,
отдельный тип репо — не сервис, не интерфейс). Stub — standalone-программа, форма
которой задаётся дескриптором из manage (`form`: контейнер / CLI / …). Out-of-band-
наблюдение collector'ом — режим: включается, когда есть наблюдаемая поверхность
(сеть/хост/процесс); иначе stub — поставляемая программа без наблюдения. Внутренняя
структура — на усмотрение под выбранный стек; `MODULE.md`/`SPEC.md` **не применяются**
...
- Брокера, `BACKLOG/specs`, presentation-эндпоинтов здесь **нет** — stub не
  участник общения и не сервис. Хаб-контракт к stub неприменим (нет потребления
  envelope).
