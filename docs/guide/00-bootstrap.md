...
**Цель:** от пустого клона скелета сервиса к репо сервиса, готовому к разработке.

**Когда:** первый вход в репо — человеком или агентом.

**Что нужно до:** скопирован `skeletons/service/` из репо методологии в новый
репо сервиса (см. `docs/INDEX.md` → *Инстанциация*).

## Шаги

1. **Создать репо сервиса** из `skeletons/service/` репо методологии:
   скопируй содержимое `skeletons/service/` в новый репо, оторви историю:
   ```bash
...
2. **Выбрать стек** (один на сервис): Python / Go / Rust / TypeScript.
   ...
   - В `docs/refs/STACKS.md` (читается из репо методологии) — оставь секцию
     выбранного стека как референс; закрепи компактную строку выбранного стека
     в `AGENTS.md` → *Команды проверки*.
