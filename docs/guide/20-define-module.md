...
**Что нужно до:** этап 10 (модуль есть в таблице `docs/ARCHITECTURE.md`).

**Применимо:** сервис и gateway-сервис. `MODULE.md` / `SPEC.md` не применяются
к интерфейсу и автономному компоненту.

**Результат:** модуль имеет канонические швы, спеку и запись в архитектуре.
...
Каталог и публичный API модуля соответствуют `MODULE`,
спека содержит семь секций, а топики и таблица модулей актуальны.
