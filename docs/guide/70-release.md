...
- **pre-release** — `main` прошёл гейт, образ собирается; режется так часто,
  как хочется держать «свежий кандидат». В автономном цикле rc режется
  **автоматически после merge** в `main` (деплой в тестовую среду = первая
...
> Уровень версии задаётся суффиксом semver: `-rc.N` (pre-release);
> без суффикса — стабильная. Других pre-release-суффиксов нет.

## Уровни версий

- **pre-release** — `vX.Y.Z-rc.N`. Режется часто, как поток кандидатов. В
...
- Тег — точка релиза; хаб ссылается на собранный образ стабильного тега
  (pre-release-теги — для внутренних прогонов).
