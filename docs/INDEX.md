...
- Приоритет **по ярусам**: `AGENTS.md` > `guide/`+`refs/` > `skeletons/` > код.
  `guide/` и `refs/` — **равны** (разные виды документа, не старшинство).
  Приоритет арбитражирует **только между ярусами** (AGENTS > canon > skeletons >
  код). Противоречие **внутри яруса** (`guide/` против `refs/`) — это **дефект**,
  а не «старший побеждает»: его чинят правкой документа к одной правде (факт в
  `refs/`, процедура ссылается, не дублирует). Сама методология — единый живой
  документ, эволюционирует коммитами, **без ADR** для своих внутренних изменений
  (ADR — процедура для инстанцированных репо: `docs/guide/60-adr.md`).