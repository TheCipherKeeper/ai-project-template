...
Версия внешнего канона закреплена в `.methodology.yml`. Агент и CI читают этот
tag, а не плавающий `main`; обновление версии выполняется отдельной задачей.
...
- **Ревью и merge — агент, не человек.** Ревьюер — независимый вызов агента;
  ...
  автоматически в пределах risk/autonomy задачи. Человек задаёт intent и
  подтверждает действия, для которых `RISK.md` требует risk-gate: **(1)** пишет программный
...
Сначала запусти policy-гейт закреплённой методологии, затем `lint / test /
build`. Недоступность обязательного гейта или роли блокирует работу. Полномочия,
риск, evidence и recovery определены центральными `POLICY.md`, `RISK.md`,
`EVIDENCE.md`, `RECOVERY.md`.
