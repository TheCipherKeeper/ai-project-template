...
<!-- Где проходит граница доверия сервиса, что по какую сторону, гарантии,
     ...
     • Если сервис — **gateway** (каноническая роль, единственный browser-facing
       surface; ровно один на систему при наличии интерфейсов): здесь же —
     ...
     • Если сервис **обычный**: browser-facing presentation здесь НЕ описывай —
       ...
       Секцию границы заполняй про брокерное/аутентификацию сервиса. -->

<!-- Только для gateway-сервиса: -->

- Presentation-эндпоинты (browser-facing; только gateway):
