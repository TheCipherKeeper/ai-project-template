...
входят, как зависят, какой брокер. Это **реестр связей** для оркестрации проверок:
обязательная проверка хаба валидирует и перечисляет дочерние репозитории отсюда,
а каждый дочерний репозиторий проверяется собственным CI на закреплённой версии.

Канонический формат строк, версий и пустых разделов задан в
`<methodology-repo>/docs/ARCHITECTURE.md` → *Рабочие артефакты*.
