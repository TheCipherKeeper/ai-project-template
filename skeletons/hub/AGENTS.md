...
- **Программный бэклог** — `BACKLOG.md` в этом репо. Единственная очередь «что
  ...
  (ровно один `[~]` in-flight). Захват — короткий lock-PR с атомарной проверкой
  актуального `main` через merge queue либо обязательный rebase/recheck;
  stale-check запрещён, только merge означает получение лока. Агенты
  читают/правят бэклог здесь — не в
...
- Держать **browser-facing presentation-эндпоинты** где-либо, кроме
  **gateway-сервиса** — только он их держит; ровно один, если есть ≥1 интерфейс
  (модель — `<methodology-repo>/docs/refs/COMMUNICATION.md` → *Сервис-шлюз*).
