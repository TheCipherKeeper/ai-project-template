...
Версия внешнего канона закреплена в `.methodology.yml`; плавающий `main` не
используется. Обновление версии методологии — отдельная задача и PR.
...
- Запускать единый policy-гейт из закреплённой версии методологии; обязательная
  проверка или роль недоступна — fail closed.
...
- Редактировать `BACKLOG.md` (программный бэклог): человек — порядок/новые
  задачи и диагностические статусы; агент — `[ ] ready`→`[~]`→`[x]` (маркер персистентен через
