...
пользовательского интерфейса**. Здесь приведены обязательные правила
...
> Репозиторий создаётся из `skeletons/interface/`. Пользовательский интерфейс
> является клиентом на доверительной границе, не подключается к брокеру и не
> выполняет роль сервиса. Он обращается только к HTTP- и WebSocket-API
> сервиса-шлюза, описанному в разделе *Доверительная граница* файла
> `ARCHITECTURE.md` шлюза. Модель приведена в
