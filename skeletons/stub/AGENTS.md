...
(standalone-программа: форма — параметр дескриптора — контейнер / CLI / …;
out-of-band-наблюдение collector'ом — режим, не свойство; не симулятор и не
брокер-клиент).
...
> Stub — **standalone-программа**, **не участник брокера** и **не peer-сервис**: не
> publish/consume топики, не имеет presentation-эндпоинтов (HTTP/WS для интерфейсов).
> Форма — параметр дескриптора из control-plane (manage): контейнер / CLI / …
> (`form`). Out-of-band-наблюдение collector'ом — **режим**, не свойство:
> включается, когда есть наблюдаемая поверхность (сеть/хост/процесс), иначе stub —
> просто поставляемая программа без наблюдения. Деплой — по форме (контейнер —
> `Dockerfile`; CLI — артефакт/сборка). `MODULE.md`/`SPEC.md` к stub **не
> применяются** (бэкенд-канон usecase-швов для сервиса); stub параметризуется
> дескриптором, а не юзкейсами.
>
...
коммита** (гейт-agent #10 блокирует; см.
...
- Писать код stub-таргета (поверхности/баннеры/поведение при наблюдении,
  параметризуемые дескриптором) и его конфигурацию.
...
- Быть **брокер-клиентом** или **peer-сервисом** — stub не publish/consume топики,
  не имеет presentation-эндпоинтов; не лезет в чужую БД/брокер. При включённом
  режиме — наблюдается collector'ом out-of-band, сам ничего не зовёт.
