...
- **`skeletons/interface/`** — стартовый набор interface-репо (React/TS,
  визуализации, потребление presentation-эндпоинтов сервисов). Копируется →
  новый репо.
...
graph LR
  ...
  H --> I["interface-репо ×M<br/>(из skeletons/interface)<br/>React/TS, визуализации"]
  M --> I
  S -. "HTTP/WS<br/>presentation" .-> I
```

- **Этот репо** — корень авторитета, не инстанцируется как сервис, не содержит
  кода приложения. Хаб/сервисы/интерфейсы ссылаются на `docs/guide/` и
  `docs/refs/` здесь.
- **Хаб-репо** хранит системные контракты (`CONVENTIONS@vN`) и состав программы
  (`COMPOSITION` — сервисы + интерфейсы); сервисы пинят версию, гейт проверяет
  сервис против пина.
- **Сервис-репо** — клиент брокера (одного на систему: Kafka/Redpanda/NATS),
  ...
  экспонирует presentation-эндпоинты для интерфейсов.
- **Интерфейс-репо** — React/TS, клиент на границе: зовёт presentation-эндпоинты
  сервисов по HTTP/WS; деплой статикой.
...
работай по фазе 30; 4) на каждый интерфейс скопируй `skeletons/interface/` →
новый репо, заполни `ARCHITECTURE` (потребляемые эндпоинты, страницы); 5)
стабильные версии — тегами `vX.Y.Z` (фаза 70).
